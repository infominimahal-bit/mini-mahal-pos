...
export function createSignIn(deps: SignInDeps) {
  const { setLoading, setProfile, setUser } = deps

  return async function signIn(identifier: string, password: string) {
    setLoading(true)
    try {
      ...
      if (isOfflineError) {
        ...
        try {
          ...
          if (matchedUser) {
            ...
            setUser({ id: matchedUser.id, email: matchedUser.email } as any);
