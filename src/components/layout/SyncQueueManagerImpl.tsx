...
import { RefreshCw, Trash2, CheckCircle2, Receipt, ShieldAlert, ChevronDown } from 'lucide-react';
...
import { getEntityIcon, getStatusBadge, getGroupStatusBadge, getGroupLabel } from './syncQueueBadges';

export interface SyncQueueManagerProps {
    onClose: () => void;
}
    ...
    const footer = (
        <div className="flex items-center gap-2 sm:gap-3 w-full">
            ...
            >
                ...
                <span>{loading ? "SYNCING..." : "FORCE RE-SYNC"}</span>
            </Button>
        </div>
    );
            ...
            <div className="flex flex-col gap-6">
                    ...
                    <div className="space-y-3 max-h-[300px] overflow-y-auto pr-1.5 custom-scrollbar min-h-[120px]">
                        ...
                        <div className="flex items-center justify-between mb-2 sticky top-0 bg-white dark:bg-surface z-10 py-1">
                            ...
                            {ops.some(o => o.status === 'failed') && (
                                <Button variant="ghost" onClick={handleClear} className="!min-h-0 !px-2 !py-1 !rounded-lg !text-[8px] !text-rose-500 !bg-rose-500/5 hover:!underline">{"Flush Stuck"}</Button>
                            )}
                        </div>
                                ...
                                <div key={batchId} className="rounded-xl border border-gray-200 dark:border-white/5 bg-[#f8f9fa] dark:bg-white/[0.02] overflow-hidden">
                                    ...
                                    {!isCollapsed && (
                                        <div className="border-t border-gray-200 dark:border-white/5">
                                            {children.map((op) => (
                                                <div key={op.id} className="group px-3 py-2 hover:bg-white dark:hover:bg-white/5 transition-all border-b border-gray-100 dark:border-white/[0.02] last:border-b-0">
                                                    ...
                                                    {op.lastError && (op.status === 'error' || op.status === 'failed') && (
                                                        <div className="mt-1.5 p-1.5 rounded-lg bg-rose-500/10 border border-rose-500/20 text-[8px] font-bold text-rose-600 dark:text-rose-400 break-words leading-tight w-full">
                                                            {op.lastError}
                                                        </div>
                                                    )}
                                                </div>
                        ...
                        {ungrouped.map((op) => (
                            <div key={op.id}>
                                <div className="group p-2.5 rounded-xl border border-gray-200 dark:border-white/5 bg-[#f8f9fa] dark:bg-white/[0.02] transition-all hover:bg-white dark:hover:bg-white/5">
                                    <div className="flex items-center justify-between gap-3">
                                        <div className="flex items-center gap-2.5 min-w-0">
                                            <div className="p-2 rounded-lg bg-white dark:bg-black/20 border border-gray-200 dark:border-white/5 shrink-0 shadow-sm">
                                                {getEntityIcon(op.entity)}
                                    ...
                                    {op.lastError && (op.status === 'error' || op.status === 'failed') && (
                                        <div className="mt-1.5 p-1.5 rounded-lg bg-rose-500/10 border border-rose-500/20 text-[8px] font-bold text-rose-600 dark:text-rose-400 break-words leading-tight">
                                            {op.lastError}
                                        </div>
                                    )}
                                </div>
                ...
                <div className="p-4 bg-gray-50 dark:bg-white/[0.03] rounded-2xl border border-gray-200 dark:border-white/5">
                    <p className="text-[9px] font-bold text-gray-600 dark:text-gray-500 uppercase tracking-[0.15em] leading-relaxed text-center">
                        {"PENDING CHANGES SYNC IN BACKGROUND. IF A CHANGE FAILS 5 TIMES, IT IS MARKED STUCK AND NEEDS MANUAL ATTENTION."}
                    </p>
                </div>
            </div>
        </Modal>
