-- ============================================================================
-- 20260820070000_commit_sale_oversell_guard
-- ----------------------------------------------------------------------------
-- GAP 1: re-add the server-side OVERSELL guard inside commit_sale. Stock must
-- never go negative unless app_settings.allow_negative_stock is true. This is
-- defense-in-depth behind the client UI guard (which is bypassable offline /
-- via concurrent tabs / direct API). Variant movements are excluded from the
-- check because their stock lives in variant_data (parent products.stock is not
-- authoritative for variants); variant oversell is guarded client-side.
-- ============================================================================
...
BEGIN
  ...
  -- OVERSELL GUARD (P3/P35): never drive product.stock negative unless explicitly allowed.
